...
function renderScheduleEmailHtml(vesselName: string, schedule: ReportSchedule, snapshot: any, openFindings: any[], recentChanges: any[]) {
  ...
  return `
    <div style="font-family:Arial,sans-serif;background:#080b10;color:#f0f4f8;padding:24px;line-height:1.6">
      ...
      ${sections.has('maritime') ? `<h2 style="font-size:16px;margin:0 0 8px;">Maritime Signal Integrity</h2><ul style="padding-left:20px;color:#dce8f4;margin:0 0 16px;"><li>${maritimeAlerts.filter((alert: any) => !alert?.resolved).length} active maritime-related alerts</li><li>${maritimeAlerts.length} maritime alerts captured in this snapshot</li>${maritimeAlertItems || '<li>No GNSS/edge exposure alerts in the latest snapshot.</li>'}</ul>` : ''}
    </div>
  `;
}
